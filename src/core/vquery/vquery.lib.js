...
class VQuery {
	 ...
	 * @param {string} selector - A CSS selector string to search for within the selected element.
	 * @returns {VQuery[]} An array of new VQuery instance for the found elements.
	 */
	findAll(selector) {
		const elements = this.element.querySelectorAll(selector)
		return Array.from(elements).map(element => new VQuery(element))
	}
	 ...
	 * @returns {VQuery|string} The current VQuery instance for chaining
	 when setting text content, or the current text content when getting.
	 */
	text(textContent) {
	...
	/**
	 * Add an event listener to the selected element for the specified
	 * event type.
	 ...
	 * @param {function(Event): void} callback - The event listener
	 ...
	 receive the event object as its argument.
	 * @returns {VQuery} The current VQuery instance for chaining.
	 */
	 ...
	 * @returns {VQuery} The current VQuery instance for
