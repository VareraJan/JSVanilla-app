...
export class StatisticsItem extends ChildComponent {
	 ...
	 * 
	 * @param {string} label - The label to be displayed in the
	 statistic item.
	 * @param {string|number} value - he value to be displayed in the
	 statistic item.
	 * @param {('purple'|'green')} variant - The variant that determines
	 the appearance of the statistic item. Allowed values:
	 'purple' or 'green'.
	 */
	...
	render() {
		...
		$V(this.element).addClass(styles[this.variant]).addClass('fade-in')
		$V(this.element).find('#statistic-label').text(this.label)
		$V(this.element).find('#statistic-value').text(this.value)
